...
/**
 * Created by gongqibing on 16/7/3.
 */
public class PedometerView extends View {
    ...
    //文字颜色
    private int textColor = Color.parseColor("#FF8ED0FF");
    //内外圈的颜色
    private int outCircleColor = Color.parseColor("#FF8ED0FF");
    private int inCircleColor = Color.parseColor("#FFD3EAFA");
    ...
    private void init(Context context, AttributeSet attrs) {
        if (null != attrs) {
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.pedometerViewStyle);
            textColor = typedArray.getColor(R.styleable.pedometerViewStyle_textColor, textColor);
            outCircleColor = typedArray.getColor(R.styleable.pedometerViewStyle_outCircleColor, outCircleColor);
            inCircleColor = typedArray.getColor(R.styleable.pedometerViewStyle_inCircleColor, inCircleColor);
            avatarDrawable = typedArray.getDrawable(R.styleable.pedometerViewStyle_avatar);
            typedArray.recycle();
        }
        //圆弧所需的矩形
        mRect = new RectF();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        setMeasuredDimension(getDimenMeasure(getSuggestedMinimumWidth(),widthMeasureSpec,0.9f)
        ,getDimenMeasure(getSuggestedMinimumHeight(),heightMeasureSpec,0.7f));
    }

    private int getDimenMeasure(int suggestedMini, int measureSpec, float scale) {
        int result = suggestedMini;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        initCanvas(w, h, oldw, oldh);
    }

    private void initCanvas(int w, int h, int oldw, int oldh){
        width = w;
        height = h;
        centerX = 0;
    ...
    private void paintBottom(Canvas canvas){
        ...
        canvas.drawBitmap(toRoundBitmap(drawableToBitmap(avatarDrawable)),null,dst,null);
